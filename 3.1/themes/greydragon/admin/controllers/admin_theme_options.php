...
class Admin_Theme_Options_Controller extends Admin_Controller {
  ...
  protected function get_edit_form_admin() {
    ...
    $group = $form->group("recommended")->label("Module Support &amp; Compatibility");

    $module_check = ((module::is_active("kbd_nav")) and (module::info("kbd_nav")));
    $this->prerequisite_check($group, "kbdnavcheck", $module_check, 
    ...
    $group->dropdown("thumb_ratio")
      ...
      ->options(array("photo" => t("Actual Size"), "digital" => t("Digital 4:3"), "film" => t("Film 3:2")/*, "square" => t("Square (cropped)")*/ ))
  ...
  protected function get_edit_form_help() {
    ...
    $help .= '<li><h3>Prerequisites</h3>
      ...
      <p>Also check that <b>Info</b> module is installed and active.</p>
      </li>';

    $help .= '<li><h3>Module Support &amp; Compatibility</h3>
      <p>Here you\'ll find information on additional features and their requirements, including those activated with companion modules.</p>
    ...
    $help .= '<li><h3>General Settings</h3>
      ...
      Number of <b>Rows per Album Page</b> however can vary. Also theme restricts thumbnails size to max 200x200px.</p>
      <p>Default G3 logo can be replaced with your own by providing <b>Alternate Logo Image</b>.
      Recommended logo size is within 300x80px. If you need bigger space for your logo, CSS would have to be adjusted.
      Otherwise, Logo could be suppressed altogether by providing <b>Header Text</b> to replace it.
      <b>Footer Text</b> would be simply placed next to Site\'s credits.</p>
      <p><b>Copyright Message</b> can be placed in the right top corner of the footer.</p>
      <p>Important feature of the theme is a support for Color Packs which is managed by <b>Selected Color Pack</b> Option.
      <b>Color Pack</b> is a small CSS file defining color rules for the theme. By default theme comes with GreyDragon (default),
      Carbon and Wind sets, but it could be easily extended. Visit Theme\'s Download page for additional information.</p>
      </li>';
    $help .= '<li><h3>Advanced Options - General</h3>
      <p>Shows appreciation for hard work of G3 team and Theme\'s author by allowing
      <b>Show Site Credits</b> and/or with your small <b>Donation</b> to our Coffee Fund.</p>
      ...
      <p>If you prefer breadcrumbs not being used, simply hide it with <b>Hide Breadcrumbs</b> option.
    ...
    $help .= '<li><h3>Advanced Options - Album page/Thumbs</h3>
      ...
      <p><b>Aspect Ratio</b> should be used with understanding that some information may be out of visible area in thumbs.
      Based on specified aspect all displayed size would be adjusted accordingly. When switching to/from <b>Actual Size</b>, it is recommended to rebuild 
      thumbs for proper display (see Maintenance section below).</p>
      <p>If you would like Item\'s caption to be visible you can manage it with <b>Title Display Mode</b>.
      And if metadata (owner/clicks/etc) is unnecessary, it could be removed with <b>Hide Item Meta Data</b>.</p>
      </li>';
    $help .= '<li><h3>Advanced Options - Photo page</h3>
      <p>Similar to Thumb\'s options above some tune-up could be done for individual Photo pages.</p>
      </li>';
  ...
  protected function reset_theme() {
    ...
    module::clear_var("th_greydragon", "thumb_ratio");
