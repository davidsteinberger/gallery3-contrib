(function($) {
	...
	function callDB(albumId, photoID){
						...
						function(transaction, results){
							if (results.rows.length > 0){
								...
								for (var n=0; n<results.rows.length; n++) {
									...
									loadAsync(albumId, photoID, newImage.encodedtext);
								}
							}
							else {
								$.ajax({
										...
										success: 
											function(resource) {
												...
												loadAsync(albumId, photoID, resource.substring(to+2, length));
										},
										dataType: 'base64'
								});
							}
	...
	function loadAsync(albumId, photoId, resource){
		...
		//canvasImg.setAttribute("id", 'cx'+photoId);
		//canvasImg.setAttribute("title", photoId);

		//drawImageOnCanvas(resource, canvasImg);
