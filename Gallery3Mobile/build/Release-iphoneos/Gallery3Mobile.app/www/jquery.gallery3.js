					...
					transaction.executeSql("Select * from ImageData where id = ? order by size;", [photoID], 
						function(transaction, results){
							if (results.rows.length > 0){
								...
								if(getConfig().items[photoID].entity.type == 'photo'){
									console.log('resize getting loaded');
									$.ajax({
										async: true,
										url: getConfig().siteUrl + "/rest/data/" + photoID + '?size=resize&encoding=base64',
										beforeSend: function(xhr) {
										xhr.setRequestHeader("X-Gallery-Request-Key", config.token);
										},
										success: 
											function(resource) {
												...
												getConfig().db.transaction(
													function (transaction) {
														...
														transaction.executeSql("INSERT INTO ImageData (id, size, base64) VALUES (?, ?, ?)", [photoID, '0', resource], _nullDataHandler, _errorHandler );
														if(callback == 'drawThumbs'){
															drawThumbs(albumId, photoID, resource);
														}
														else if (callback == 'drawThumb'){
															drawThumb(albumId, photoID, resource);
														}	
													}
												);
										},
									...
									console.log('thumb getting loaded');
									$.ajax({
										async: true,
										url: getConfig().siteUrl + "/rest/data/" + photoID + '?size=thumb&encoding=base64',
										beforeSend: function(xhr) {
										xhr.setRequestHeader("X-Gallery-Request-Key", config.token);
										},
										success: 
											function(resource) {
												...
												getConfig().db.transaction(
													function (transaction) {
														transaction.executeSql("DELETE FROM ImageData where id = ?", [photoID]);
														transaction.executeSql("INSERT INTO ImageData (id, size, base64) VALUES (?, ?, ?)", [photoID, '1', resource], _nullDataHandler, _errorHandler );
														if(callback == 'drawThumbs'){
															drawThumbs(albumId, photoID, resource);
														}
														else if (callback == 'drawThumb'){
	...
	function drawThumbs(albumId, photoId, resource){
		...
		$('<img id="' + photoId + '" class="' + ''  + '" alt="'+ '' +'" title="'+ 'description' +'"/>').load(function(){
			...
			*/
			...
			var $li = $('<li />',{
				id : photoId,
				className	: 'pic ' + getConfig().items[photoId].entity.type
			});
			var $a = $('<a/>',{
				href	:	'aldi' + photoId
			});
