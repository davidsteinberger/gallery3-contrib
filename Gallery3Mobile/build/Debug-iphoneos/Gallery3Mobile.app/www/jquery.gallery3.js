					...
					transaction.executeSql("Select * from ImageData where id = ? order by size;", [photoID], 
						function(transaction, results){
							if (results.rows.length > 0){
								...
								$.ajax({
										async: true,
										url: getConfig().siteUrl + "/rest/data/" + photoID + '?size=resize&encoding=base64_2',
										beforeSend: function(xhr) {
										xhr.setRequestHeader("X-Gallery-Request-Key", config.token);
										},
										success: 
											function(resource) {
												...
												getConfig().db.transaction(
													function (transaction) {
														...
														transaction.executeSql("INSERT INTO ImageData (id, size, base64) VALUES (?, ?, ?)", [photoId, '0', resource.substring(to+2, length)] );
													}
												);
												if(callback == 'drawThumbs'){
													...
													success: 
														function(resource) {
															...
															getConfig().db.transaction(
																function (transaction) {
																	transaction.executeSql("DELETE FROM ImageData where id = ?", [photoId]);
																	transaction.executeSql("INSERT INTO ImageData (id, size, base64) VALUES (?, ?, ?)", [photoId, '1', resource.substring(to+2, length)] );
																}
															);
															if(callback == 'drawThumbs'){
																drawThumbs(albumId, photoID, resource.substring(to+2, length));
